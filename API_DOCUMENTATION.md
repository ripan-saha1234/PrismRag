...
9. [Tracked Company Pages APIs (Live Website Scraping)](#7-tracked-company-pages-apis-live-website-scraping)
   - [`GET /api/admin/pages`](#get-apiadminpages)
   - [`POST /api/admin/pages`](#post-apiadminpages)
   - [`PUT /api/admin/pages/:id`](#put-apiadminpagesid)
...
## 7. Tracked Company Pages APIs (Live Website Scraping)

These endpoints manage company website pages that the AI assistant can reference live without pre-ingesting into Qdrant. Pages can be plain static HTML or client-side rendered React SPAs. Pages are periodically refreshed (every 3 hours) and served instantly from PostgreSQL cache (`page_content_cache`).

### `GET /api/admin/pages`
Lists all tracked website pages with their metadata, fetch status, error logs, and cache timestamps.
...
[
  {
    ...
    "page_type": "auto",
    "is_active": true,
    "last_fetched_at": "2026-09-05T17:00:00.000Z",
    "last_fetch_status": "success",
    "last_fetch_error": null,
    "cache_updated_at": "2026-09-05T17:00:00.000Z"
  }
]
```
...
Registers a new company website page to be tracked and fetched.
...
- **Body Schema**:
  ```json
  {
    ...
    "page_type": "string (optional: 'auto' | 'static' | 'react', default: 'auto')"
  }
  ```
...
| Field | Type | Required | Rules & Validation |
| :--- | :--- | :--- | :--- |
| `label` | String | **Yes** | Non-empty label identifying the page. |
| `url` | String | **Yes** | Unique URL. Returning `409 Conflict` if URL is already tracked. |
| `page_type` | String | No | `'auto'`, `'static'`, or `'react'`. Defaults to `'auto'`. |
...
- **`409 Conflict`**:
  ```json
  {
    "error": "A page with this URL is already tracked."
  }
  ```
...
Updates an existing tracked page's label, URL, page type, or active state.

#### Request
- **Method**: `PUT`
- **URL**: `/api/admin/pages/:id`
- **Headers**: `Content-Type: application/json`
- **Body Schema**: Any combination of `{ label, url, page_type, is_active }`.

#### Success Response (`200 OK`)
```json
{
  "id": 2,
  "label": "Updated Pricing Plans",
  "url": "http://localhost:5173/pricing",
  "page_type": "auto",
  "is_active": true,
  "last_fetched_at": "2026-09-05T17:00:00.000Z",
...
- **`409 Conflict`**:
  ```json
  {
    "error": "Another page with this URL already exists."
  }
  ```
...
Deletes a tracked page from PostgreSQL.
...
### `POST /api/admin/pages/:id/test-fetch`
Executes an immediate live fetch/render of the URL **without saving to cache**. Returns extracted body text so administrators can preview the scraper results before relying on it.

#### Request
- **Method**: `POST`
- **URL**: `/api/admin/pages/:id/test-fetch`
...
```json
{
  ...
  "url": "http://localhost:5173/pricing",
  "page_type": "react",
  "extracted_length": 1435,
  "extracted_text": "Transparent Engineering Plans\nPredictable pricing designed for ambitious startups..."
}
```
...
### `POST /api/admin/pages/refresh-all`
Forces an immediate on-demand background refresh of all active tracked pages using `Promise.allSettled`.

#### Request
- **Method**: `POST`
- **URL**: `/api/admin/pages/refresh-all`
...
```json
{
  ...
  "summary": [
    { "id": 1, "label": "landing", "status": "success" },
    { "id": 2, "label": "services", "status": "success" },
    { "id": 3, "label": "pricing", "status": "success" }
  ]
}
```
...
| `selected_option`| `TEXT` | Nullable (or Not Null) | Single answer option chosen by user. |
| `selected_options`| `JSONB` | Nullable (legacy schema) | Array of chosen option(s) for backwards-compatibility. |
...
| Column | Type | Constraints | Description |
| :--- | :--- | :--- | :--- |
| `id` | `SERIAL` | Primary Key | Unique tracked page ID. |
| `label` | `VARCHAR` | Not Null | Human-readable name (e.g. `'Pricing Plans'`). |
| `url` | `TEXT` | Not Null, Unique | Target page URL to scrape/render. |
| `page_type` | `VARCHAR` | Default `'auto'` | Scraping mode: `'auto'`, `'static'`, or `'react'`. |
| `is_active` | `BOOLEAN` | Default `TRUE` | Whether active for LLM tool inspection and scheduled refreshes. |
| `page_content_cache`| `TEXT` | Nullable | Last successfully fetched page text body. |
| `cache_updated_at` | `TIMESTAMPTZ` | Nullable | When cache was last successfully refreshed. |
